			...
			<ol class="property-list facebookUser">
				...
				<li class="fieldcontain">
					<span id="user-label" class="property-label"><g:message code="facebookUser.user.label" default="User" /></span>
						...
						<span class="property-value" aria-labelledby="user-label"><g:link controller="usuario" action="show" id="${facebookUserInstance?.user?.id}">${facebookUserInstance?.user?.encodeAsHTML()}</g:link></span>
